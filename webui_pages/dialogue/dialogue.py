...
from configs import (TEMPERATURE, HISTORY_LEN, PROMPT_TEMPLATES, LLM_MODELS,
                     ...
                     PROJECTS,SDP_RATIO,NOISE_SCALE,NOISE_SCALE_W,LENGTH_SCALE,
                     DEFAULT_EMOTION,DEFAULT_PROJECT)
...
def dialogue_page(api: ApiRequest, is_lite: bool = False):
    ...
    # st.session_state['audio_generator'] = AudioGenerator(DEFAULT_PROJECT)
    ...
    def convert_text_to_audio(text:str):
        ...
        try:
            ...
            for t in text_chunks:
                if t == "":
                    continue
                audio_data_part,params['sample_rate'] = st.session_state['audio_generator'].convert_text_to_audio(t,
                                                                                                        dialogue_speaker,
                                                                                                        sdp_ratio,
                                                                                                        noise_scale,
                                                                                                        noise_scale_w,
        ...
        if parse_command(text=prompt, modal=modal):  # 用户输入自定义命令
            ...
            if dialogue_mode == "LLM 对话":
                ...
                if dialogue_project != "无":
                    audio_element = convert_text_to_audio(text)
                    if audio_element != None:
                        chat_box.update_msg(audio_element, element_index=2,streaming=False)
                    else:
                        chat_box.update_msg("语音生成失败! 请检查报错文件log.txt!", element_index=2,streaming=False)
